...
app_ui = ui.page_sidebar(
    ui.sidebar(
        ui.input_slider("frame", "Frame of animation", min=1, max=512, value=0, step=1, animate=True),
        ...
        ui.accordion(
            ui.accordion_panel('Numerics Settings',
                               ui.input_slider("num_frames", "Number of frames in simulation", min=124, max=2048, value=512, step=16),
                               ui.input_slider("dtau", "Proper time step (s)", min=0.01, max=1.0, value=0.1, step=0.01),
                               ),
            open=False,
        ),
        open='always'
...
def server(input, output, session):
    ...
    @reactive.effect
    def _():
        ui.update_slider("frame", max=input.num_frames())
    ...
    def _():
        ...
        with ui.Progress(min=0, max=num_frames-1) as p:
    ...
    def plot():
        ...
        with plt.style.context(style_label):
            ...
            frame_idx = input.frame() - 1
            frame_data = lambda: data.iloc[frame_idx]
            ax.scatter(data['position'], data['time'], c=data['velocity'], vmin=-1, vmax=1, cmap=cmap)
            ax.plot(data['position'], data['time'], color='grey')

            times = ax.get_yticks()[2:-1]  # Exclude first and last tick to avoid clutter
            # Interpolate positions of moving observer at these times to plot self times
            times_at_this_proper_time = np.interp(times, data['proper_time'], data['time'], left=np.nan, right=np.nan)
            positions_at_times = np.interp(times_at_this_proper_time, data['time'], data['position'], left=np.nan, right=np.nan)
            ...
            ax.axline((frame_data()['position'], frame_data()['time']), slope=frame_data()['velocity'], color=red)
            ax.axline((frame_data()['position'], frame_data()['time']), slope=np.tan(np.pi/2 - np.atan(frame_data()['velocity'])), color=red)
            ...
            xlim = ax.get_xlim()
            ylim = ax.get_ylim()
            ax.set_xlim(xlim[0] - 1, xlim[1] + 7)
            ax.set_ylim(ylim[0] - 2, ylim[1] + 2)
            ax.set_aspect('equal')

            for t, x, tau in zip(times_at_this_proper_time, positions_at_times, times):
                ax.text(x+1, t, f'τ={tau:.0f}s', color=red, verticalalignment='center', horizontalalignment='left')
                ax.scatter(x, t, color=red, marker='x')
